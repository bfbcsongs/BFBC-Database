...
let db = null;
let currentLoadedSongId = null; // Track active song in Chord Studio
...
// Scoped Audio Player Loader & Studio Synchronizer
window.loadYTPlayer = function(songId, videoId) {
    currentLoadedSongId = songId;

    // Load into the Chord Recorder Studio URL bar and trigger playback
    ...
    if (typeof loadVideoFromInput === 'function') {
        loadVideoFromInput();
    }

    // Restore saved recorded chords if available for this song
    const targetSong = songs.find(s => s.id == songId);
    if (targetSong && Array.isArray(targetSong.chords) && typeof recordedChords !== 'undefined') {
        recordedChords = [...targetSong.chords];
        if (typeof renderMarkers === 'function') renderMarkers();
        if (typeof renderRecordingsList === 'function') renderRecordingsList();
    }

    // Scroll to the studio top smoothly
...
// ==========================================
// CHORD DATABASE BINDING DISPATCHER
// ==========================================
window.saveChordsToDatabase = async function(chordArray) {
    let targetId = currentLoadedSongId;

    if (!targetId) {
        if (songs.length === 0) {
            alert('No songs available to attach chords to.');
            return;
        }
        const songTitles = songs.map((s, index) => `${index + 1}. ${s.title}`).join('\n');
        const choice = prompt(`Select a song number to save these chords to:\n\n${songTitles}`);
        ...
        targetId = songs[selectedIndex].id;
    }

    const song = songs.find(s => s.id == targetId);
    if (!song) {
        alert('Target song not found.');
        return;
    }

    if (db) {
        try {
            const { error } = await db.from('songs_sandbox').update({ chords: chordArray }).eq('id', targetId);
            if (error) {
                alert('Save Chords Failed: ' + error.message);
                return;
            }
            alert(`Successfully saved ${chordArray.length} chords for "${song.title}"!`);
            await fetchSongs();
        } catch (err) {
            alert('Database Connection Error: ' + err.message);
        }
    } else {
        song.chords = chordArray;
        alert(`Chords temporarily saved locally for "${song.title}"!`);
    }
};
        ...
        <div class="bg-slate-800 border border-slate-700/70 rounded-xl overflow-hidden transition-all mb-2">
                ...
                <div class="flex items-center gap-3 overflow-hidden">
                    ...
                    <div class="truncate">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <span class="text-[10px] font-semibold text-slate-400 bg-slate-900 px-2 py-0.5 rounded-full inline-block">${song.category}</span>
                            ${chordCount > 0 ? `<span class="text-[10px] font-bold text-emerald-400 bg-emerald-950/80 border border-emerald-500/30 px-2 py-0.5 rounded-full inline-block"><i class="fa-solid fa-guitar text-[9px]"></i> ${chordCount} Chords</span>` : ''}
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-2 shrink-0">
            ...
            <div id="accordion-details-${song.id}" class="hidden p-4 border-t border-slate-700/60 bg-slate-900/40 space-y-3">
                <div class="flex items-center gap-2 flex-wrap">
                    ${ytId ? `
                    <button onclick="loadYTPlayer('${song.id}', '${ytId}')" class="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600/20 text-rose-400 hover:bg-rose-600 hover:text-white border border-rose-600/30 rounded-lg text-xs font-semibold transition-all cursor-pointer">
                        <i class="fa-solid fa-play"></i> Play Studio Audio
                    </button>` : ''}
                ...
                <div class="mt-2 rounded-lg overflow-hidden border border-slate-700 bg-slate-900">
                    <div id="yt-preview-${song.id}" class="relative cursor-pointer group" onclick="loadYTPlayer('${song.id}', '${ytId}')">
                        ...
                        <div class="absolute inset-0 bg-black/40 flex items-center justify-center">
                            <div class="px-4 py-2 bg-rose-600/90 text-white text-xs font-bold rounded-full flex items-center gap-2 shadow-lg group-hover:scale-105 transition-all">
                                <i class="fa-solid fa-play"></i> Tap to Load into Chord Studio
                            </div>
                        </div>
                    </div>
                </div>
...
// ==========================================
// SAVE CHORDS TO SUPABASE
// ==========================================
window.saveChordsToDatabase = async function(chordArray) {
    if (!chordArray || !Array.isArray(chordArray) || chordArray.length === 0) {
        alert('No chords to save!');
        return;
    }
    ...
    if (db) {
        try {
            ...
            alert(`Saved ${chordArray.length} chords for "${song ? song.title : 'Selected Song'}"!`);
            await fetchSongs();
        } catch (err) {
            alert('Database Connection Error: ' + err.message);
        }
